function param = Parameters_LG_INR21700_M50_comsolValidation
% Parameters_LG_INR21700_M50_comsolValidation loads all the nominal DFN parameters obtained 
...
%--------------------------------------------------------------------------
% Open-circuit voltage function: U(cs_surf/cs_max) [V]
%--------------------------------------------------------------------------
param.Up = @Up_NMC811_comsolValidation;
param.Un = @Un_Graphite_Si_comsolValidation;
...
param.sigman=NaN;       
% Solid phase conductivity at cathode [S/m]
param.sigma_effp=0.18;
% Solid phase conductivity at anode [S/m]   
param.sigma_effn=215;
...
%--------------------------------------------------------------------------
param.De_eff = @De_eff_Chen2020_comsolValidation; 
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% Option 1: Functional electrolyte conductivity coefficient: kappa_eff(ce) [S/m]
%--------------------------------------------------------------------------
param.Kappa_eff = @Kappa_eff_Chen2020_comsolValidation;
%--------------------------------------------------------------------------
% Option 2: Constant electrolyte conductivity coefficient: kappa_eff [S/m]
%--------------------------------------------------------------------------
% param.Kappa = 0.95;
% param.Kappa_eff = @Kappa_eff_constant;

%--------------------------------------------------------------------------
% Option 1: Functional transference number function: t1(ce) [-]
%--------------------------------------------------------------------------
% param.t1 = @t1_Kremer;
%--------------------------------------------------------------------------
% Option 2: Constant functional transference number: t1 [-]
%--------------------------------------------------------------------------
param.t1_constant = 0.2523;
param.t1= @t1_constant;

%--------------------------------------------------------------------------
% Option 1: Functional activity term: lambda(ce) [-]
%--------------------------------------------------------------------------
% param.lambda = @lambda_Kremer; 
%--------------------------------------------------------------------------
% Option 2: Scalar activity term: lambda [-]
%--------------------------------------------------------------------------
param.lambda_constant = 1;
param.lambda= @lambda_constant;

%--------------------------------------------------------------------------
% Kinetic parameters [m2.5/(mol0.5.s)]
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% Perturbation time (time allowed for perturbation approach to find consistent IC)
param.initime = 3; % [s]
