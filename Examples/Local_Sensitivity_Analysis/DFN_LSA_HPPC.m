...
% DFN_LSA_HPPC.m
% -------------------------------------------------------------------------
%   DFN_LSA_HPPC performs local sensitivity analysis (LSA) for HPPC by perturbing 
%   each parameter one-at-a-time. This script can be used to determine sensitive
%   parameters for the HPPC profile. Once the sensitive parameters are determined, they can be 
%   identified using HPPC experimental data using "DFN_pso_HPPC.m" script.
%   This script takes longer to run than DFN_LSA_CC.m since we are simulating HPPC profile.
...
theta_names = {'$D_{s,p}$' '$D_{s,n}$' '$t_+$' '$k_p$' '$k_n$' '$c_0$' '$D_e$' '$\kappa$'};
