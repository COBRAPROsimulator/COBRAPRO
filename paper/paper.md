...
- **Solution:** The single step approach [@lawder_extending_2015], a robust initialization method, is implemented in COBRAPRO that determines the initial conditions by modifying the AEs into implicit ODEs and seamlessly integrates the consistent initial conditions to solve the DFN model. As shown in [@ha_cobrapro_2024], the single-step approach is able to determine consistent initial conditions more robustly than using a typical nonlinear algebraic solver, such as SUNDIALS IDA CalcIC or CasADI's rootfinder function, which are used in the other open-source codes. 
