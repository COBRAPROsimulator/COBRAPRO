...
- **Solution:** The single-step approach [@lawder_extending_2015], a robust initialization method implemented in COBRAPRO, modifies the AEs into implicit ODEs to determine consistent initial conditions. This approach eliminates the need for nonlinear algebraic solvers, such as SUNDIALS IDA CalcIC or CasADi's rootfinder, which are commonly used by other open-source DFN codes but have been shown to have convergence issues [@methekar_perturbation_2011, @lawder_extending_2015]. In [@ha_cobrapro_2024], a comparison study between the single-step approach and SUNDIALS IDA CalcIC demonstrates that the single-step approach consistently determines initial conditions across different C-rates and node numbers. For details on the implementation of the single-step approach, see [@ha_cobrapro_2024].
